...
// ====== Registro global de presets externos ======
const customPresets = new Map();

export function registerPreset(name, fn) {
  if (typeof fn !== 'function') {
    console.warn(`GraphGeometry.registerPreset: fn must be a function`);
    return;
  }
...
export class GraphGeometry extends THREE.Group {
  constructor(type, ...params) {
    ...
    // ====== Chequeo de presets externos ======
    if (customPresets.has(type)) {
      const { nodes: n = [], edges: e = [] } = customPresets.get(type)(...params) || {};
      nodes = n;
      edges = e;
    } else {
      // ====== Presets internos ======
